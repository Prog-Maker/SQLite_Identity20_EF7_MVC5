...
namespace Identity.SQLite
{
    ...
    /// </summary>
    public class AppUserStore<TUser> : IUserLoginStore<TUser>,
        ...
        where TUser : IdentityUser
    {
        ...
        /// </summary>
        public AppUserStore(IdentityEntities identityEntities)
        {
        ...
        public async Task<IList<string>> GetRolesAsync(TUser user)
        {
            ...
            var currentUserRoles= await _userRolesRepository.FindUserRolesByUserIdAsync(user.Id);
        ...
        public async Task<bool> IsInRoleAsync(TUser user, string roleName)
        {
            ...
            var role = await _identityEntities.AspNetRoles.SingleOrDefaultAsync(r => r.Name.ToUpper() == roleName.ToUpper());
            if (role != null)
            {
                var userId = user.Id;
                var roleId = role.Id;
                return await _identityEntities.AspNetUserRoles.AnyAsync(ur => ur.RoleId.Equals(roleId) && ur.UserId.Equals(userId));
            }
            return false;
        }
        ...
        public async Task RemoveFromRoleAsync(TUser user, string role)
        {
            ...
            await _userRolesRepository.RemoveUserRoleAsync(user.Id, role);
        }
        ...
        public async Task<bool> HasPasswordAsync(TUser user)
        {
            ...
            return !String.IsNullOrWhiteSpace(await _userRepository.GetPasswordHashAsync(user.Id));
        }
